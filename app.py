...
# === OTMAN API SETTINGS ===
# Special thanks to OTMAN for this API
...
# === OTMAN GUEST ACCOUNT ===
OTMAN_UID = "4929669504"
OTMAN_PASSWORD = "otman-C6SNRZHDRJoT"
...
def get_account_credentials(region: str) -> str:
    """جميع المناطق تستخدم حساب OTMAN"""
    return f"uid={OTMAN_UID}&password={OTMAN_PASSWORD}"
...
def home():
    return jsonify({
        ...
        "example": "/player-info?uid=123456789"
    })
...
def refresh_tokens_endpoint():
    try:
        ...
        return jsonify({'message': 'Tokens refreshed for all regions.','developer': 'OTMAN'}), 200
    except Exception as e:
        return jsonify({'error': f'Refresh failed: {e}'}), 500

# === Startup ===
async def startup():
    print("🚀 OTMAN Free Fire API Starting...")
    print(f"📱 Using Guest Account: {OTMAN_UID}")
    await initialize_tokens()
    asyncio.create_task(refresh_tokens_periodically())
    print("✅ OTMAN API is ready!")
